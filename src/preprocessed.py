...
SAVE_COLUMNS = [
    "raw_index","message_type","is_sender","message","sticker_path",
    "CreateTime","unix_seconds","time_gap"
]
TYPE_MAPPING = {
    "文本":"text",
    "表情包":"sticker",
...
def process_chat_csv(df: DataFrame) -> DataFrame:
    ...
    if invalid_time_count:
        raise ValueError(f"CreateTime中存在{invalid_time_count}条无法解析的时间")
...
def drop_empty_messages(
    ...
    sticker_valid = df[message_type_col].eq("sticker")
...
def add_message_id(df: DataFrame, id_col: str = "message_id", prefix: str = "msg") -> DataFrame:
    if "raw_index" not in df.columns:
        raise ValueError("缺少raw_index列，无法生成稳定message_id")
    result = df.copy().reset_index(drop=True)
    result[id_col] = result["raw_index"].apply(lambda value: f"{prefix}_{int(value):08d}")
    return result
